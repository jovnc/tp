...
The UI consists of a `MainWindow` that is made up of parts e.g.`CommandBox`, `ResultDisplay`, `PlayerListPanel`,
...
* stores the currently 'selected' `Player` objects (e.g., results of a search query) as a separate _filtered_ list which
  is exposed to outsiders as an unmodifiable `ObservableList<Player>` that can be 'observed' e.g. the UI can be bound to
...
`Player` references. This allows `AddressBook` to only require one `Tag` object per unique tag, instead of each `Player`
...
Step 2. The user executes `delete 5` command to delete the 5th player in the address book. The `delete` command calls
...
Step 3. The user executes `add n/David …​` to add a new player. The `add` command also calls
...
Step 4. The user now decides that adding the player was a mistake, and decides to undo that action by executing the
...
| `* * *`  | user                                       | add a new player             |                                                                        |
| `* * *`  | user                                       | delete a player              | remove entries that I no longer need                                   |
| `* * *`  | user                                       | find a player by name        | locate details of players without having to go through the entire list |
| `* *`    | user                                       | hide private contact details | minimize chance of someone else seeing them by accident                |
| `*`      | user with many players in the address book | sort players by name         | locate a player easily                                                 |
